...
> {
  ...
  private organizationRequestChange = (
    ...
    // selectedHospitalsNew.add(22);
    // return { isChecked: false, selectedHospitalsNew: updatedSelection };

    this.setState((prevState: any) => ({
      ...prevState,
      pdfDivisionIDs: updatedDivisionIds,
      // selectedHospitalsNew: selectedHospitalsNew
    }));
  };
  ...
  private getStatus = (
    ...
    matrix: any,
    key: string,
    key2: string // Second key for the additional check
  ) => {
    ...
    if (a) {
      switch (true) {
        // Check for 'key1' and 'key2' values
        case a[key] === "G" && a[key2] === "U":
  ...
  private generatePrintFuctionRequest = async () => {
    ...
    try {
      const response = await fetch(
        "https://systemgoalapi.bilh.org/Print/api/report/",
        {
          method: "POST",
          headers: {
  ...
  public render(): React.ReactElement<ISystemGoalKpiProps> {
    ...
    console.log("aAaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa ---->", dataGoalMetrix);
                        ...
                        >
                          ...
                          >
                            <input
                              ...
                              checked={organization.division.every(
                                (divison: any) =>
                                  divison.hospitals.every((hospital: any) =>
                                    selectedHospitalsNew.has(hospital.id)
                                  )
                      ...
                      <div>
                        <div className="system_goel_container">
                          ...
                          {goalHirerachyData.map((goal: any) => (
                            <div
                              className={`box_model ${
                                !checkedSystemGoalsNew.has(goal.id)
                                  ? "d-none"
                                  : ""
                              }`}
                            >
                                                  ...
                                                  <>
                                                    ...
                                                    {(allDivisionSelected ||
                                                      ...
                                                      <tr className="organization_avg">
                                                        <td
                                                          data-tooltip-id={`tooltip-${organization.id}_${kpi.id}`}
                                                          style={{
                                                            textAlign: "left",
                                                          }}
                                                        >
